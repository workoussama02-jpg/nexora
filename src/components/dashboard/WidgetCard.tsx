...
export default function WidgetCard({ widget, onEdit, onDownload, onDelete, downloading }: WidgetCardProps) {
  return (
    <div className="group rounded-xl border border-white/10 bg-white/5 p-5 transition hover:border-brand-primary/30 hover:bg-white/[0.07]">
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-white">{widget.name}</h3>
        <p className="mt-1 text-sm text-gray-400">Created {formatDate(widget.created_at)}</p>
      </div>

      <div className="flex items-center gap-2">
        <button
          onClick={() => onEdit(widget.id)}
          className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm text-gray-300 transition hover:bg-white/10"
          title="Edit widget"
        >
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm text-gray-300 transition hover:bg-white/10 disabled:opacity-50"
